...
class U_Net(object):
    ...
    def _build_model(self):
        immy_a, _, _, immy_a_sem = self.build_input_image_op(
            self.input_list_train, False)
        self.input_images, self.input_sem_gt = tf.train.shuffle_batch(
            [immy_a, immy_a_sem], self.batch_size, 100, 30, 8)

        self.input_sem_pred = u_net_model(
            self.input_images, self.options, False, name='u_net')
        self.sem_loss = self.criterionSem(
            self.input_sem_pred, self.input_sem_gt, self.num_classes)
        self.sem_loss_sum = tf.summary.scalar(
        ...
        self.val_sem_pred = u_net_model(
            self.val_images, self.options, True, name='u_net', is_test=True)
    ...
    def build_input_image_op(self, input_list_txt, is_test=False, num_epochs=None):
        ...
        if not is_test:
            ...
            crop_offset_w = tf.cond(tf.equal(tf.shape(image)[1] - self.crop_size_w, 0), lambda: 0, lambda: tf.random_uniform(
            ...
            image_sem = tf.image.crop_to_bounding_box(
                image_sem, crop_offset_h, crop_offset_w, self.crop_size_h, self.crop_size_w)
            image.set_shape([None, None, 3])
            image_sem.set_shape([None, None, 1])
    ...
    def accuracy_validation(self, args):
        ...
        for idx in range(self.num_sample_test):
            print("Evaluating accuracy on validation set", idx + 1, "/",
                  self.num_sample_test, end='\r' if idx != self.num_sample_test - 1 else '\n')
    ...
    def train(self, args):
        ...
        in_pred = tf.expand_dims(
            tf.argmax(self.input_sem_pred, axis=-1), axis=-1)
        val_pred = tf.expand_dims(
            tf.argmax(self.val_sem_pred, axis=-1), axis=-1)

        tf.summary.image('train', self.input_images,
                         max_outputs=1, collections=["TRAINING_IMAGES"])
        tf.summary.image('train sem gt', color(self.input_sem_gt),
                         max_outputs=1, collections=["TRAINING_IMAGES"])
        tf.summary.image('train sem pred', color(in_pred),
                         max_outputs=1, collections=["TRAINING_IMAGES"])

        tf.summary.image('val', self.val_images, max_outputs=1,
                         collections=["VALIDATION_IMAGES"])
        tf.summary.image('val sem gt', color(self.val_sem_gt),
                         max_outputs=1, collections=["VALIDATION_IMAGES"])
        tf.summary.image('val sem pred', color(val_pred),
                         max_outputs=1, collections=["VALIDATION_IMAGES"])

        summary_scalar_train_op = tf.summary.merge(
            tf.get_collection("TRAINING_SCALAR"))
        summary_scalar_val_op = tf.summary.merge(
            tf.get_collection("VALIDATION_SCALAR"))
        summary_images_train_op = tf.summary.merge(
            tf.get_collection("TRAINING_IMAGES"))
        summary_images_val_op = tf.summary.merge(
            tf.get_collection("VALIDATION_IMAGES"))
        ...
        while self.counter <= self.num_epochs*self.num_sample:
            ...
            if (self.counter // self.num_sample) < ((self.counter + self.batch_size)//self.num_sample) and self.counter != 0:
                ...
                if mean_acc >= self.best:
                    print("Accuracy step ", self.counter, ": ",
                          mean_acc, " old best: ", self.best)
                    self.best = mean_acc
                    self.save(self.saver_best, os.path.join(
                        args.checkpoint_dir, "best"), self.counter)

                #### summary writing ####
                summary_string = self.sess.run(summary_scalar_val_op, feed_dict={
                                               self.accuracy_placeholder: mean_acc})
            ...
            if self.counter % (30*self.batch_size) == 0 and self.counter != 0:
                summary_string1, summary_string2 = self.sess.run(
                    [summary_images_train_op, summary_images_val_op])
    ...
    def load(self, checkpoint_dir):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            self.counter = int(q)
            savvy = tf.train.Saver(
                var_list=get_var_to_restore_list(ckpt.model_checkpoint_path))
    ...
    def test(self, args):
        """Test"""
        sample_op, sample_path, im_shape, sample_op_sem = self.build_input_image_op(
            args.input_list_val_test, is_test=True, num_epochs=1)
        sample_op, sample_path, im_shape, sample_op_sem = tf.expand_dims(sample_op, axis=0), tf.expand_dims(
            sample_path, axis=0), tf.expand_dims(im_shape, axis=0), tf.expand_dims(sample_op_sem, axis=0)

        sem_images = u_net_model(
            sample_op, self.options, name='u_net', is_test=True)
        sem_images_out = tf.argmax(sem_images, axis=-1, name="prediction")
        sem_images_out = tf.cast(tf.expand_dims(
            sem_images_out, axis=-1), tf.uint8)
        ...
        while count <= args.num_sample_test:
            ...
            pred_sem_imgs, sample_images, sample_paths, im_sps, sem_gt = self.sess.run(
                [sem_images_out, sample_op, sample_path, im_shape, sample_op_sem])
            # iterate over each sample in the batch
            # create output destination
            print(sample_paths[0])
            # sample_paths[rr].decode('UTF-8').replace("./testA",args.test_dir)
            dest_path = os.path.join(args.test_dir, sample_paths[0].decode(
                'UTF-8').split("/")[-1].replace(".jpg", ".png"))
            parent_destination = os.path.abspath(
                os.path.join(dest_path, os.pardir))
    ...
    def inference(self, args):
        """Test"""
        image = tf.placeholder(dtype=tf.float32, shape=(1, None, None, 3))
        #image = tf.image.convert_image_dtype(image, tf.float32)
        #image = (image*2)-1

        sem_images = u_net_model(
            image, self.options, name='u_net', is_test=True)
        sem_images_out = tf.argmax(sem_images, axis=-1, name="prediction")
        sem_images_out = tf.cast(tf.expand_dims(
            sem_images_out, axis=-1), tf.uint8)
        ...
        # test_img = tf.read_file(
        #     '/home/daniele/data/datasets/kitti/2011_10_03_drive_0027_sync/image_02/cropped/0000000000.jpg')
        # test_img = tf.image.decode_image(test_img, channels=3)
        # test_img = tf.expand_dims(test_img, axis=0)
        # _, h, w, d = test_img.eval().shape
        ...
        pred_sem_imgs, sample_images = self.sess.run(
            [sem_images_out, image], feed_dict={image: test_img})

        pred_sem_img = misc.imresize(np.squeeze(
            pred_sem_imgs[0], axis=-1), (h, w))

        # im_sp = im_sps[0]
        # pred_sem_img = misc.imresize(np.squeeze(
        #     pred_sem_imgs[0], axis=-1), (im_sp[0], im_sp[1]))

        print(np.max(pred_sem_img))
        print(np.min(pred_sem_img))

        #print(pred_sem_img.shape, pred_sem_img[200:250, 400:450])
        import matplotlib.pyplot as plt
        misc.imsave("/tmp/ciao.png", pred_sem_img)
    ...
    def buildRuntimeInferenceModel(self, args):
        """Test"""
        image = tf.placeholder(dtype=tf.float32, shape=(1, None, None, 3))
        #image = tf.image.convert_image_dtype(image, tf.float32)
        #image = (image*2)-1
